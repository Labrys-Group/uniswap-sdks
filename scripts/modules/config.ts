 ...
 */
export function validateConfig(config: any): asserts config is WhitelabelConfig {
  ...
  if (typeof config.chainName !== 'string' || config.chainName.trim().length === 0) {
  ...
  const validateAddress = (key: string, value: any) => {
    if (typeof value === 'string') {
      if (!addressPattern.test(value)) {
        error(`addresses.${key} must be a valid Ethereum address (got: ${value})`)
      }
    } else if (value && typeof value === 'object' && 'address' in value) {
      if (!addressPattern.test(value.address)) {
        error(`addresses.${key}.address must be a valid Ethereum address (got: ${value.address})`)
      }
